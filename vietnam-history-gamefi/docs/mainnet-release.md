...
- `GET /health/ready` trả HTTP 503 khi RPC, mint, metadata, program upgrade authority, reward config/vault, database hoặc đối soát đang có vấn đề. `scripts/check-mainnet-readiness.py` kiểm tra thêm đường báo giá Jupiter ở cả hai chiều; chỉ in trạng thái và số liệu công khai.
...
2. Chuẩn bị RPC Mainnet riêng, PostgreSQL production, HTTPS origin và Jupiter API key. Chạy `database/migrations/001_dex_swaps.sql` và `002_reward_claims.sql`. Chép mẫu env vào file `.env` được Git bỏ qua; không đưa key vào biến `VITE_*`.
...
5. Khởi tạo reward distributor với admin multisig, distributor signer, vault và hạn mức; nạp ngân sách theo quyết định treasury. Kiểm tra PDA, số dư vault và thử một payout nhỏ. Mainnet phải dùng quy trình ký/rotation có giám sát trước khi mở thưởng thật.
